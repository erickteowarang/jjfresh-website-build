...
echo "${boldwhite}Please note: This script is designed to work with dudestack and jjfresh-website. See https://github.com/digitoimistodude${txtreset} "
...
for i in `grep -rl jjfresh-website * 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;jjfresh-website;${THEMENAME};" $i $i; done
for i in `grep -rl jjfresh-website * 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;jjfresh-website;${THEMENAME};" $i $i; done
for i in `grep -rl air * 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;jjfresh-website;${THEMENAME};" $i $i; done
...
On top of features included in [digitoimistodude/jjfresh-website](https://github.com/digitoimistodude/jjfresh-website), this project contains:
